...
def get_orbital_animation_html(
    center_dot_size: int = 16,
    orbit_dot_size: int = 12,
    orbit_radius: int = 30,
    num_orbit_dots: int = 3,
    center_color: str = "#76b900",
    orbit_color: str = "#76b900",
    animation_duration: float = 2.0,
    container_size: int = 100,
    enable_3d_depth: bool = True,
) -> str:
    """
    Generate HTML/CSS for a simple orbital animation with dots orbiting a center dot

    Args:
        center_dot_size: Size of the center dot in pixels
        orbit_dot_size: Size of each orbiting dot in pixels
        orbit_radius: Radius of the orbit in pixels
        num_orbit_dots: Number of dots orbiting around the center (2-6 recommended)
        center_color: Color of the center dot (hex or CSS color)
        orbit_color: Color of the orbiting dots (hex or CSS color)
        animation_duration: Duration of one complete orbit in seconds
        container_size: Size of the animation container in pixels
        enable_3d_depth: Whether to enable 3D depth effects (elliptical orbit, scaling, opacity)

    Returns:
        HTML string with embedded CSS for the orbital animation
    """
    # Calculate positions for orbiting dots
    orbit_dots_html = ""
    for i in range(num_orbit_dots):
        angle_offset = (360 / num_orbit_dots) * i
        delay = (animation_duration / num_orbit_dots) * i
        orbit_dots_html += f'''
        <div class="orbit-dot orbit-dot-{i}" style="
            animation-delay: -{delay}s;
            --start-angle: {angle_offset}deg;
        "></div>'''

    # Choose animation keyframes based on 3D depth setting
    orbit_keyframes = (
        """
    @keyframes orbit3d {
        0% {
        ...
        25% {
            transform:
                rotate(calc(var(--start-angle, 0deg) + 90deg))
                translateX(var(--orbit-radius))
                scaleY(0.6)
                scale(1.1)
                rotate(calc(-1 * (var(--start-angle, 0deg) + 90deg)));
            opacity: 1;
            z-index: 3;
        }
        50% {
            transform:
                rotate(calc(var(--start-angle, 0deg) + 180deg))
                translateX(var(--orbit-radius))
                scaleY(0.4)
                scale(0.7)
                rotate(calc(-1 * (var(--start-angle, 0deg) + 180deg)));
            opacity: 0.4;
            z-index: 1;
        }
        75% {
            transform:
                rotate(calc(var(--start-angle, 0deg) + 270deg))
                translateX(var(--orbit-radius))
                scaleY(0.6)
                scale(1.1)
                rotate(calc(-1 * (var(--start-angle, 0deg) + 270deg)));
            opacity: 1;
            z-index: 3;
        }
        100% {
    ...
    @keyframes orbit2d {
        0% {
            transform: rotate(var(--start-angle, 0deg)) translateX(var(--orbit-radius)) rotate(calc(-1 * var(--start-angle, 0deg)));
        }
        100% {
            transform: rotate(calc(var(--start-angle, 0deg) + 360deg)) translateX(var(--orbit-radius)) rotate(calc(-1 * (var(--start-angle, 0deg) + 360deg)));
        }
    }
    """
        if enable_3d_depth
    ...
    animation_name = "orbit3d" if enable_3d_depth else "orbit2d"
    center_shadow = "box-shadow: 0 2px 8px rgba(0,0,0,0.2);" if enable_3d_depth else ""
    dot_shadow = "box-shadow: 0 1px 4px rgba(0,0,0,0.3);" if enable_3d_depth else ""

    return f"""
    <style>
    :root {{
        --center-dot-size: {center_dot_size}px;
        --orbit-dot-size: {orbit_dot_size}px;
    ...
    .orbit-dot {{
        ...
        top: 50%;
        left: 50%;
        margin-top: calc(-1 * var(--orbit-dot-size) / 2);
        margin-left: calc(-1 * var(--orbit-dot-size) / 2);
        animation: {animation_name} var(--animation-duration) linear infinite;
        transform-origin: 0 0;
        {dot_shadow}
    }}
    </style>
